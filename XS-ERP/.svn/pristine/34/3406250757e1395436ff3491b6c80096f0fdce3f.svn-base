$(function() {
	$.ajax({
		...
		success : function(data) {
			...
			var str ="<li><a title='搜索'  href='#'><img alt='搜索' src='/images/search.png' width='25px' height='25px'></a></li>"+
               "<li><a id='toggle-sidebar' title='设置' onclick='topRight();'><img alt='头像' src='/images/people.png' width='25px' height='25px'></a></li>"
			...
			for(var i =0;i<data.length;i++){
				...
				for(var j=0;j<data.length;j++){
					if(data[j].menuDepth==3 && data[j].parentId==data[i].id){
						child+="<li class='file folderTwo'><a onclick=\"addTabs('"+data[j].menuName+"','"+data[j].menuPath+"');\">"+data[j].menuName+"</a></li>";
					}
				}
				if(data[i].menuDepth==2){
					if(child!=""){
						$('.tree').append("<li><label for='folder"+i+"' class='folderOne'>"+data[i].menuName+"</label> <input type='checkbox' id='folder"+i+"'/><ol>"+child+"</ol></li>");
						$(".tree").trigger("create");
					}else{
						$('.tree').append("<li><label for='folder"+i+"' class='folderOne' onclick=\"addTabs('"+data[i].menuName+"','"+data[i].menuPath+"');\">"+data[i].menuName+"</label> <input type='checkbox' id='folder"+i+"'/></li>");
						$(".tree").trigger("create");
					}
	...
	$(".tree").on("click","label",function(){
	    ...
	    var isChecked = $("label").next("input[type='checkbox']").is(':checked');
	    //展开和收齐的不同状态下更换右侧小图标
	    if(isChecked){
	        $(this).css(
...
function topRight(){
	var str ="<div id='setting' style='display:none;width:80px'>"+
			"<a class='btn btn-default btn-block' style='width:105px;'><strong>修改个人简介</strong></a>"+
			"<a class='btn btn-default btn-block' style='width:105px;'><strong>修改头像</strong></a>"+
			"<a class='btn btn-default btn-block' style='width:105px;'><strong>修改昵称</strong></a>"+
			"<a class='btn btn-default btn-block' style='width:105px;'><strong>修改密码</strong></a>"+
			"<a class='btn btn-default btn-block' style='width:105px;'><strong>注销</strong></a></div>";
	$.messager.show({
        msg:str,
        showType:'show',
        width:140,
