...
public class EmployeeController extends BaseController {
	...
	public String editEmployee(Model model, @RequestParam int id) {
		...
		model.addAttribute("eItem", eItem);
		model.addAttribute("listDepItem", toJson(departmentService.selectAllDepInfoByEmpID(id)));
		return "employee/edit";
	}
	...
	public RspResultDto addEmployee(EmployeeItem employeeItem, int[] depID) {
		return employeeService.addEmployeeDepartment(depID, employeeItem, getUserID());
	}

	@RequestMapping(value = "/employee/update", produces = "application/json;charset=UTF-8")
	@ResponseBody
	public RspResultDto editEmployee(EmployeeItem employeeItem, int[] depID) throws ParseException {
		return employeeService.updateEmployeeDepartment(employeeItem, getUserID(), depID);
	}
	...
	@ResponseBody
	public RspResultDto updatePwd(EmployeeItem employeeItem) {
	...
	@ResponseBody
	public RspResultDto uploadPicture(@RequestParam(value = "file", required = false) MultipartFile file, HttpServletRequest request, HttpServletResponse response) {
	...
	@ResponseBody
	public PageInfo<EmployeeItem> getQueryList(@RequestParam(value = "depID", required = false) String depID, @RequestParam(value = "empNo", required = false) String empNo,
			@RequestParam(value = "startTime", required = false) String startTime, @RequestParam(value = "endTime", required = false) String endTime,
			@RequestParam(value = "page", required = false) int page, @RequestParam(value = "rows", required = false) int rows) {
		return employeeService.selectEmployeeByPage(page, rows, empNo, depID, startTime, endTime);
	}
