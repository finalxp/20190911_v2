...
public class text1 {
	private static String sourepath = "C:\\ti_plp2covv2";
	private static String outputpath = "D:\\newVoice";

	public static void main(String[] args) {
		splitWav(sourepath, outputpath);
		long feature = 2;
	...
	public static void splitWav(String Filepath, String output) {
		...
		if (files.exists()) {
			...
			if (null == fileArr || fileArr.length == 0) {
				...
				for (File file : fileArr) {
					...
					String[] infomation = file.getName().split("-");
					String phone = infomation[2];
					if (filename.contains(phone)) {
						fileutils.copy(file, new File(output, phone));
					}
				}
			}
		}
